...
async function syncTelegramMiniApp(){
 ...
 try{
  ...
  const r=await fetch('https://api.telegram.org/bot'+token+'/setChatMenuButton',{
   ...
   body:JSON.stringify({menu_button:{type:'web_app',text:'Открыть Шаурмег',web_app:{url:String(process.env.CLIENT_MINI_APP_URL||'https://shaurma-city-app.onrender.com/?source=telegram&b=82').trim()}}})
  });
  const j=await r.json().catch(()=>({}));
