...
async function getAdminBotInfo(){
 if(adminBotInfo)return adminBotInfo;
 const token=adminTelegramBotToken();if(!token)return null;
 const r=await fetch('https://api.telegram.org/bot'+token+'/getMe');
 const j=await r.json().catch(()=>({}));
 if(!r.ok||!j.ok||!j.result?.username)throw new Error(j.description||('HTTP '+r.status));
...
async function createSharedAccessCode(establishmentId,createdBy){
 ...
 const est=String(establishmentId||'').trim().toUpperCase();
 if(!/^SC-MSK-[A-F0-9]{10}$/.test(est))throw new Error('bad_establishment_id');
 const venue=await DB.query('SELECT establishment_id,name FROM shaurma_venues WHERE establishment_id=$1 AND is_active=TRUE LIMIT 1',[est]);
 if(!venue.rows[0])throw new Error('venue_not_found');
 const code='OWN-'+crypto.randomBytes(5).toString('hex').toUpperCase();
 const permissions=['menu','profile','media','appearance','orders'];
 const q=await DB.query(`INSERT INTO shaurma_venue_invites(establishment_id,code_hash,role,permissions,expires_at,max_uses,is_active,created_by,kitchen_enabled)
   VALUES($1,$2,'owner',$3::jsonb,NOW()+INTERVAL '7 days',1,TRUE,$4,TRUE)
   RETURNING id,establishment_id,expires_at`,
...
async function adminVenueKeyPage(chatId,page=0,editMessageId=null,api=adminTelegramApi){
 ...
 const rows=q.rows.slice(0,size),hasNext=q.rows.length>size;
 const keyboard=rows.map(x=>[{text:'🥙 '+String(x.name||x.establishment_id).slice(0,44),callback_data:'ak:'+x.establishment_id}]);
 const nav=[];if(p>0)nav.push({text:'← Назад',callback_data:'akp:'+(p-1)});if(hasNext)nav.push({text:'Дальше →',callback_data:'akp:'+(p+1)});if(nav.length)keyboard.push(nav);
...
async function handleAdminBotUpdate(update,api=adminTelegramApi){
     ...
     try{
       ...
       return api('sendMessage',{
         ...
         reply_markup:{inline_keyboard:[
           ...
           [{text:'🔑 Другой ключ',callback_data:'akp:0'}]
         ]}
       });
     }catch(e){
 ...
 if(/^\/start(?:@[A-Za-z0-9_]+)?/i.test(text)||/^\/admin(?:@[A-Za-z0-9_]+)?$/i.test(text)){
   return api('sendMessage',{
     ...
     reply_markup:{inline_keyboard:[
       [{text:'🔑 Ключи доступа',callback_data:'akp:0'}],
       [{text:'🗺 Открыть админку',web_app:{url:PUBLIC_API_URL+'/shaurma-owner?v=4'}}]
     ]}
   });
 }
}
...
function normalizeAstraRealCityConfig(raw={}){
 const src=raw&&typeof raw==='object'&&!Array.isArray(raw)?raw:{};
 const statuses=new Set(['collecting','ready_for_astra','processing','review','approved']);
 return {
  version:1,
  mode:'digital_twin_exact',
  status:statuses.has(String(src.status||''))?String(src.status):'collecting',
  accuracy:'maximum',
  preserve_map_geometry:true,
  radius_m:Math.max(60,Math.min(350,Number(src.radius_m)||190)),
  main_building_enabled:src.main_building_enabled!==false,
  panorama_enabled:src.panorama_enabled!==false,
  include_street_objects:src.include_street_objects!==false,
  include_vegetation:src.include_vegetation!==false,
  include_signage:src.include_signage!==false,
  notes:String(src.notes||'').trim().slice(0,2500),
  astra_instruction:String(src.astra_instruction||'').trim().slice(0,4000),
  updated_at:String(src.updated_at||new Date().toISOString()).slice(0,40)
 };
}
...
function astraManifest(row){
 ...
 return {
  schema:'shaurmeg.astra.realcity.digital_twin.v1',
  target:{
   marker_id:String(row?.id||''),establishment_id:String(row?.establishment_id||''),venue_id:String(row?.venue_id||''),
   name:String(row?.name||''),address:String(row?.address||''),coordinates:{lat:Number(row?.lat),lon:Number(row?.lon)}
  },
  objective:'Create a geometrically anchored digital twin of the real venue and its surrounding district, matching the supplied references as closely as possible.',
  spatial_contract:{
   preserve_map_geometry:true,
   building_footprints_are_spatial_truth:true,
   keep_marker_coordinates:true,
  ...
  requested_output:{
   ...
   environment:'accurate neighboring buildings, roads, sidewalks, vegetation, parking, fences and street objects inside radius',
   radius_m:config.radius_m,
   lod:'optimized for iPhone Telegram Mini App',
   persistence:'durable deterministic RealCity profile reproducible after reload'
  }
 };
}
...
function adminOrderFilter(query,{includeStatus=true}={}){
 const where=[],params=[];
 const add=(sql,val)=>{params.push(val);where.push(sql.replace('?',String.fromCharCode(36)+params.length))};
...
app.get('/api/shaurma/admin/orders',async(req,res)=>{
 ...
 try{
  ...
  if(DB){
   const flt=adminOrderFilter(req.query);
   const count=await DB.query('SELECT COUNT(*)::int AS count FROM shaurma_orders'+flt.where,flt.params);
   const params=[...flt.params,limit,offset];
   const rows=await DB.query('SELECT * FROM shaurma_orders'+flt.where+' ORDER BY '+orderBy+' LIMIT '+String.fromCharCode(36)+(flt.params.length+1)+' OFFSET '+String.fromCharCode(36)+(flt.params.length+2),params);
...
app.get('/api/shaurma/admin/order-analytics',async(req,res)=>{
 ...
 try{
  if(!DB){
   ...
   for(const o of live){
    const key=new Date(o.created_at).toISOString().slice(0,10),t=timeline.get(key)||{bucket:key,orders:0,revenue:0};t.orders++;t.revenue+=Number(o.total)||0;timeline.set(key,t);
   ...
   const lifeRevenue=life.reduce((s,x)=>s+(Number(x.total)||0),0),periodRevenue=live.reduce((s,x)=>s+(Number(x.total)||0),0);
   return res.json({
    lifetime:{orders:allRows.length,sales:life.length,revenue:lifeRevenue,avg_check:life.length?lifeRevenue/life.length:0,first_order_at:allRows.map(x=>x.created_at).sort()[0]||null,last_order_at:allRows.map(x=>x.created_at).sort().slice(-1)[0]||null},
    period:{orders:periodRows.length,sales:live.length,cancelled:periodRows.filter(x=>x.status==='cancelled').length,revenue:periodRevenue,avg_check:live.length?periodRevenue/live.length:0,new:periodRows.filter(x=>x.status==='new').length,cooking:periodRows.filter(x=>x.status==='cooking').length,ready:periodRows.filter(x=>x.status==='ready').length,done:periodRows.filter(x=>x.status==='done').length},
  ...
  const products=await DB.query(`
   ...
   GROUP BY 1 ORDER BY qty DESC,revenue DESC LIMIT 100`,pf.params);
