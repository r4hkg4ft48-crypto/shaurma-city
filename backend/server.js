...
async function bootstrapRealCityProfiles(){
 if(!DB)return;
 try{
  const q=await DB.query("SELECT id FROM shaurmeg_markers WHERE is_active=TRUE AND (realcity_status<>'ready' OR COALESCE((realcity_profile->>'version')::int,0)<7) ORDER BY updated_at DESC LIMIT 24");
...
async function syncTelegramMiniApp(){
 ...
 try{
  ...
  const r=await fetch('https://api.telegram.org/bot'+token+'/setChatMenuButton',{
   ...
   body:JSON.stringify({menu_button:{type:'web_app',text:'Открыть Шаурмег',web_app:{url:String(process.env.CLIENT_MINI_APP_URL||'https://shaurma-city-app.onrender.com/?source=telegram&b=77').trim()}}})
  });
  const j=await r.json().catch(()=>({}));
