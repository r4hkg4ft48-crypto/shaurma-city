(() => {
  ...
  $('#openMenu').onclick=()=>{if(!selected)return;const u=new URL('menu.html',location.href);u.searchParams.set('marker',selected.id);u.searchParams.set('establishment',selected.establishment_id);location.href=u.toString()};
  try{tg?.ready();tg?.expand();tg?.setHeaderColor?.('#0b0f12');tg?.setBackgroundColor?.('#0b0f12')}catch{}
